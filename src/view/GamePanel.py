...
Clase GamePanel
##################################
Encargada de mostrar la interfáz del tablero
"""
import wx
...
class GamePanel(wx.Panel):
    ...
    :param config: diccionario con las configuraciones del tablero.
    :type config: dict
    """
    ...
    def onClick(self, event):
        """
        Encargada de recibir el evento de click en una carta y aplicar toda la lógica.
    ...
    def scaleBitmap(self, bitmap, width, height):
        ...
        :param bitmap: Recibe un bitmap (png,jpg, etc)
        :type parent: Bitmap
    ...
    def timeCounter(self, e):
        """
        Actuliza el contador de tiempo cada vez que es llamada por un evento.

        :param e: evento generado por wx.Time
        :type e: wx.EVT_Timer
        """
        self.time -= 1
        self.minutes = self.time // 60
    ...
    def endGame(self):
        """
        Termina el juego, aplicando la lógica de guardar jugador.
        """
        self.pointLabelCounter.SetLabel(f"{self.points}")
    ...
    def onLeaveClick(self, e=None):
        """
        Cierra el juego, si es llamada por un evento solicita confirmación
    ...
    def calculateTotalPoints(self):
        """
        Calcula el total de puntos según la dificultad elegida

        :return total: total de puntos.
        :rtype: int
        """
        total = 0
        total += self.points
